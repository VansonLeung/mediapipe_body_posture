...
- Missing body tracking for **1.8 seconds**, or hiding the page, pauses practice. Earned counts remain. After **45 seconds** without visible shoulders the kiosk saves any activity and returns to movement selection. Completion offers another attempt or another movement; navigation does not start cameras in the teacher workspace.
...
Kiosk hand navigation supports chest-up and seated framing: both shoulders and one raised wrist must be visible. Cursor mapping uses shoulder width, so hips and feet can leave the image without changing the control area. A 1.6× cursor gain maps the central 62.5% of the reachable hand area to the whole screen, reducing the movement needed to reach edge buttons; smoothing and hold-to-select remain enabled. Lower the hand between selections. Upper-body presence keeps the menu/session from timing out; it does not qualify as exercise tracking. Starting or resuming still requires a stable full-body view (including feet), and losing that view during exercise pauses progress. Seated navigation does not convert standing exercises into seated exercises.
